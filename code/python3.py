# -*- coding: utf-8 -*-
"""
Created on Fri Sep  3 10:00:06 2021
...
print("Output #17: {0:s}".format("""You can also use triple double quotations for multi-line comment strings"""))   
...
my_list_sorted_by_index_3_0 = sorted(my_lists, key=itemgetter(3,0))
...
print("Output #95: {}".format(len(my_tuple[1])))
...
print("Output #105: anotger_dict also has {!s} elements".format(len(another_dict)))
...
print("Output #122 (order by keys): {}".format(ordered_dict3))
ordered_dict4 = sorted(dict_copy.items(), key=lambda x: x[1],reverse=False)
print("Output #123 (order by keys): {}".format(ordered_dict4))
...
my_data2 = [(1,2,3),(4,5,6),(7,8,9),(1,2,3)]
set_of_tuples1_1 = {x for x in my_data2}
print("Output #134-1 (set comprehension): {}".format(set_of_tuples1_1))
...
# EXCEPTIONS
# Calculate the mean of a sequence of numeric value
def getMean(numericValues):
